...
def get_bert_embeddings(text_series, output_file, max_len=128, batch_size=64):
    if os.path.exists(output_file):
        print(f"🔄 {output_file} gevonden, laden...")
        return np.load(output_file)

    print(f"⚙️  Genereer BERT-embeddings voor {len(text_series)} items...")
    embeddings = []
    ...
    with torch.no_grad():
        for i in range(0, len(text_series), batch_size):
            ...
            print(f"🔹 Batch {i//batch_size + 1} / {(len(text_series) - 1)//batch_size + 1} verwerkt...")

    result = np.vstack(embeddings)
    np.save(output_file, result)
    print(f"✅ Embeddings opgeslagen als {output_file}")
    return result
...
# === 7. BERT-embeddings voor overview-kolom (met caching) ===
X_train_bert = get_bert_embeddings(train["overview"], "X_train_bert.npy")
X_val_bert = get_bert_embeddings(val["overview"], "X_val_bert.npy")
X_test_bert = get_bert_embeddings(test["overview"], "X_test_bert.npy")
...
# === 15. Model opslaan ===
model.save("neural_network_model_optimalised.keras")
print("✅ Neural network model opgeslagen als 'neural_network_model_optimalised.keras'.")
