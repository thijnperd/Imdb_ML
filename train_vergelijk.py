...
# ===== Vergelijkingsmodellen bouwen, trainen, opslaan en evalueren =====
...
# Model 1: getraind met MSE
model_mse = build_model(input_dim)
model_mse.compile(optimizer=Adam(0.001), loss=MeanSquaredError(), metrics=[MeanAbsoluteError()])
history_mse = model_mse.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=250, batch_size=50, verbose=1)
model_mse.save("vergelijkingsmodel_mse.keras")

# Model 2: getraind met Huber
model_huber = build_model(input_dim)
model_huber.compile(optimizer=Adam(0.001), loss=Huber(delta=1.0), metrics=[MeanAbsoluteError()])
history_huber = model_huber.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=250, batch_size=50, verbose=1)
model_huber.save("vergelijkingsmodel_huber.keras")

# === Evaluatie van beide modellen met beide losses ===
loss_functions = {
...
for model_name, model_file in {
    "Model (getraind met MSE)": "vergelijkingsmodel_mse.keras",
    "Model (getraind met Huber)": "vergelijkingsmodel_huber.keras"
}.items():
    model_loaded = tf.keras.models.load_model(model_file, compile=False)
    results[model_name] = {}
    for loss_name, loss_fn in loss_functions.items():
        model_loaded.compile(optimizer=Adam(0.001), loss=loss_fn, metrics=[MeanAbsoluteError()])
        loss, mae = model_loaded.evaluate(X_test, y_test, verbose=0)
        results[model_name][loss_name] = {"Loss": loss, "MAE": mae}

# === Resultaten printen ===
print("\n📊 Evaluatieoverzicht:")
for model_name, evals in results.items():
    print(f"\n🔸 {model_name}")
    for loss_fn, scores in evals.items():
        print(f"  - Evaluatie met {loss_fn}: Loss = {scores['Loss']:.4f}, MAE = {scores['MAE']:.4f}")

# === Loss geschiedenis uit beide modellen combineren voor visualisatie ===
plt.figure(figsize=(10,6))
plt.plot(history_mse.history['loss'], label='Training loss (MSE model)')
plt.plot(history_mse.history['val_loss'], label='Validation loss (MSE model)')
plt.plot(history_huber.history['loss'], label='Training loss (Huber model)')
plt.plot(history_huber.history['val_loss'], label='Validation loss (Huber model)')
plt.title('Training en Validatieverlies - MSE vs Huber')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.show()
